...
print(f"**La quantification n'est pas l'explication.** Sur les mêmes cellules et "
      f"avec les mêmes graines, le build `Q4_K_M` perd {fr(control)} de ses matchs "
      f"contre {fr(original)} pour le build `Q4_0`. Il échoue "
      f"{'plus souvent' if control > original else 'au moins autant'}, et non "
      f"moins : le format 4 bits le plus grossier ne peut donc pas être ce qui "
      f"empêche phi3 de tenir un format de réponse. Restent le modèle lui-même et "
      f"l'âge de son build, que ce volet ne sépare pas.")
```
